 ...
 * The Class Clock.
 */
public class Clock extends PopUpItem {
	 ...
	 * @param y the y
	 */
	public Clock(IPlayer player, World world, IHud hud, TextureAtlas powerups, float x, float y) {
		super(player, world, hud, powerups, x, y);						
	   
		frames = new Array<TextureRegion>();
        for(int i = 0; i < 3; i++){
            frames.add(new TextureRegion(powerups.findRegion("clock"), i*32, 0, 32, 32));
               }
	...
	public void update(float dt) {
        ...
        if(scheduledForRemoval && !deadWorldPopUpItem){
            ...
            setRegion(new TextureRegion(powerups.findRegion("clock"), 32, 0, 32, 32));
