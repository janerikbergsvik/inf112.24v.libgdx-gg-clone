...
/**
 * The Class SingleLightning.
 */
public class SingleLightning extends PopUpItem {
	private float timeout;

	/**
	 * Instantiates a new single lightning.
	 *
	 ...
	 * @param y the y
	 */
	public SingleLightning(IPlayer player, World world, IHud hud, TextureAtlas powerups, float x, float y) {
		super(player, world, hud, powerups, x, y);		
	   
		frames = new Array<TextureRegion>();
        for(int i = 0; i < 3; i++){
            frames.add(new TextureRegion(powerups.findRegion("single_lightning"), i*32, 0, 32, 32));
               }
	...
	public void update(float dt) {
        ...
        if(scheduledForRemoval && !deadWorldPopUpItem){
            ...
            setRegion(new TextureRegion(powerups.findRegion("single_lightning"), 32, 0, 32, 32));
        ...
        timeout -= dt;
        player.setSingleLightningTimeout(timeout);
        if(timeout <= 0) {
        	player.setShootActive(false);
        	timeout = 99999;
        }	
	}
	...
	public void use() {
		...
		player.setCheatShootActive(false);
		timeout = 20;
	}
	}
